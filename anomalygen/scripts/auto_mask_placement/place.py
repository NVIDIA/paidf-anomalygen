...
# Single generation
python3 -m anomalygen.scripts.auto_mask_placement.place \
...
python3 -m anomalygen.scripts.auto_mask_placement.place -h
"""
...
def _get_args():
    ...
    n_group.add_argument(
        "--n_range", nargs=2, type=int, metavar=("MIN", "MAX"), help="Random N range [min, max] per seed"
    )
    ...
    seed_group.add_argument(
        "--seed_range", nargs=2, type=int, metavar=("START", "END"), help="Seed range [start, end] (inclusive)"
    )
    ...
    parser.add_argument("--strict_alignment", action="store_true", help="Enable strict alignment mode")
    ...
    parser.add_argument(
        "--min_area", type=int, default=10, help="Minimum area threshold for ROI filtering (default: 10)"
    )
    parser.add_argument(
        "--max_retry_per_mask",
        type=int,
...
def _get_seed_list(args) -> List[Optional[int]]:
...
def _get_n_mapping(args, seeds: List[Optional[int]]) -> Dict[Optional[int], int]:
...
def _process_single_seed(args, seed: Optional[int], n_value: int, output_dir: str, show_progress: bool = True) -> bool:
...
def _run_single_seed_wrapper(args_tuple):
    """Wrapper for parallel processing"""
    args, seed, n_value, output_dir = args_tuple
    start_time = time.time()
    try:
        _process_single_seed(args, seed, n_value, output_dir, show_progress=False)
...
def main():
    ...
    seeds = _get_seed_list(args)
    n_mapping = _get_n_mapping(args, seeds)
    ...
    if is_single_seed:
        ...
        start_time = time.time()
        success = _process_single_seed(args, seed, n_value, output_dir, show_progress=True)
        elapsed = time.time() - start_time
        ...
        if args.parallel_workers == 1:
            ...
            with ProcessPoolExecutor(max_workers=args.parallel_workers) as executor:
                futures = {executor.submit(_run_single_seed_wrapper, task): task[1] for task in tasks}
                ...
                for i, future in enumerate(as_completed(futures), 1):
                    ...
                    if success:
                        successful_seeds.append(seed)
                        log.info(
                            f"[{i}/{len(seeds)}] Seed {seed_str} (N={n_mapping[seed]}) completed in {elapsed:.1f}s"
                        )
                    else:
                        failed_seeds.append(seed)
                        log.error(
                            f"[{i}/{len(seeds)}] Seed {seed_str} (N={n_mapping[seed]}) failed after {elapsed:.1f}s"
                        )
                        if error:
