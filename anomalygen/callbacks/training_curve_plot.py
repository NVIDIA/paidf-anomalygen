...
Plots FID, NN, MNN trajectories on a twin-y-axis figure. When ``triggered``,
the criteria metric is highlighted with best/stop markers and a patience
window. When no ``fid`` series is present, the empty FID twin-axis is hidden.
"""
...
_COLORS = {
    ...
    "mnn": "#34d399",
    "fid": "#f87171",
    "completeness": "#a78bfa",  # violet
    "precision": "#fbbf24",  # amber
    "boundary_iou": "#fb7185",  # rose
}


def _display_name(metric: str) -> str:
    """Upper-case the metric key for display."""
    return metric.upper()
...
def plot_early_stop(
    ...
    *,
    ...
    # FID is optional in this project; when absent we suppress the empty twin axis.
    has_fid = "fid" in metrics
    ...
    if triggered:
        ...
        if missing:
            raise ValueError(f"triggered=True requires non-None: {', '.join(missing)}")

        if criteria not in metrics:
            raise ValueError(f"criteria '{criteria}' not in metrics keys {list(metrics)}")
        if best_iteration not in iters:
            raise ValueError(f"best_iteration={best_iteration} is not in iters")
        if stop_iteration not in iters:
            raise ValueError(f"stop_iteration={stop_iteration} is not in iters")

    # Broken-axis mode only when early stop fired far from max_iter.
    skipped = max_iter_planned - stop_iteration if triggered else 0
    use_break = triggered and stop_iteration > iters[0] and skipped / max_iter_planned >= 0.2
    ...
    if use_break:
        ...
        # Break markers (diagonal slash) on the inner spines of both panels.
        d = 1.0  # slash aspect ratio (1.0 ~ 45deg)
        slash_kw = dict(
            marker=[(-1, -d), (1, d)],
            markersize=10,
            linestyle="none",
    ...
    # --- draw metric curves ---
    # When triggered, the criteria curve is emphasised over the others.
    for name, values in metrics.items():
        ...
        for i, ax in enumerate(target_axes):
            if triggered:
                ...
                label_first = f"{_display_name(name)}  ★ criteria" if is_criteria else _display_name(name)
            else:
                linewidth = 1.8
                alpha = 0.95
                markersize = 5.0
    ...
    if triggered:
        ...
        # Pin both markers to the criteria curve's actual y-values.
        best_y = criteria_values[best_idx]
        stop_y = criteria_values[iters.index(stop_iteration)]

        def _fmt(name: str, val: float) -> str:
            # FID: 2 decimals; nn/mnn in [0, 1]: 4 decimals.
            return f"{val:.2f}" if name == "fid" else f"{val:.4f}"

        best_summary = "   ".join(
            f"{_display_name(name)}={_fmt(name, metrics[name][best_idx])}" + (" ★" if name == criteria else "")
            for name in ("fid", "nn", "mnn")
            if name in metrics
        )
        best_label = f"best: iter={best_iteration}\n  {best_summary}"

        # When best and stop coincide, size the markers so both stay visible.
        ...
        criteria_ax.scatter(
            ...
            marker="X",
            edgecolors="white",
            linewidths=1.0 if overlap else 1.3,
        ...
        # Patience window: shaded from the last improvement through
        # ``stop_iteration``. Fall back to ``iters[-patience]`` when
        # ``last_improved_iteration`` is not supplied.
    ...
    if has_fid:
        ...
        # No FID series: hide the empty twin axis.
        for ax in fid_axes:
            ax.set_ylabel("")
    ...
    # Group metric curves first, then diagnostic-info entries.
    metric_first_tokens = {"nn", "mnn", "fid", "completeness", "precision", "boundary_iou"}
    ...
    if use_break:
        ...
        # Place legend in the empty strip past stop_iteration.
        ...
        # No trigger: park the legend outside the plot on the right.
        ax2.legend(
            handles,
            labels,
            loc="center left",
