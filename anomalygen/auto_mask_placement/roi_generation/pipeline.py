...
from anomalygen.auto_mask_placement.roi_generation.grayscale_to_mask import (
    GrayscaleToMaskPostProcess,
    grayscale_binarize,
)
from anomalygen.auto_mask_placement.roi_generation.template_box_to_masks import (
    build_template_box_to_masks_stages,
    load_cached_context,
)


def _sample_resize(image, boxes, max_size=1024):
    """
...
def prepare_samples(samples):
    ...
    For each sample:
      ...
      - Merge its `config` (if provided) with ROIGenerationConfig.
    ...
    base_schema = OmegaConf.structured(ROIGenerationConfig)
...
def ensure_valid_boxes(raw_boxes, img_w, img_h):
    ...
    for box in raw_boxes:
        ...
        if not (0 <= x0 < img_w and 0 <= x1 <= img_w and 0 <= y0 < img_h and 0 <= y1 <= img_h):
            raise ValueError(f"Box {box!r} is out of image bounds. Image size = ({img_w} * {img_h}).")
    return True


@torch.inference_mode()
def run_roi_pipeline(samples, output_dir, roi_generate_models):
    ...
    for idx, sample in enumerate(samples, start=1):
        ...
        if config.grayscale_to_mask.enabled:
            _run_grayscale_to_mask(ctx, config)
        if config.box_to_mask.enabled:
            _run_box_to_mask(ctx, config, roi_generate_models)
        if config.template_box_to_masks.enabled:
            _run_template_box_to_masks(ctx, config, roi_generate_models)

        log.info(f"[Sample {idx}/{total}] Done ({time.perf_counter() - t0:.3f}s)")


def _run_grayscale_to_mask(ctx, config):
...
def _run_box_to_mask(ctx, config, roi_generate_models):
    post_process = BoxToMaskPostProcess(config)

    t_sam = time.perf_counter()
    resized_img, resized_boxes = _sample_resize(
...
def _run_template_box_to_masks(ctx, config, roi_generate_models):
