...
def validate_sample_config(cfg):
    ...
    if op not in supported:
        raise ValueError(f"Invalid post_process.morphological_operation: {op}. Supported operations are: {supported}.")
    ...
    if gcfg.threshold_mode not in valid_modes:
        raise ValueError(f"grayscale_to_mask.threshold_mode must be one of {valid_modes}, got '{gcfg.threshold_mode}'")
