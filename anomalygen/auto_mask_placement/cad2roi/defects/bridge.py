...
def _check_touches_regions(mask: np.ndarray, regions: List[np.ndarray], min_count: int = 2) -> bool:
    """Check that mask overlaps with at least min_count regions."""
    n = sum(1 for r in regions if cv2.bitwise_and(mask, r).sum() > 0)
    return n >= min_count


def _check_single_component(mask: np.ndarray) -> bool:
...
def _validate_placement(placed, roi_mask, sub_regions):
    ...
    if not _check_touches_regions(placed, sub_regions, 2):
        return None
    clipped = cv2.bitwise_and(placed, roi_mask)
    if clipped.sum() == 0:
        return None
    if not _check_touches_regions(clipped, sub_regions, 2):
        return None
    if not _check_single_component(clipped):
        return None
    return clipped
...
def get_bridge_groups(
    components: dict, img_shape: tuple, classes: tuple = ("pad", "solder"), min_area_abs: int = 5
) -> List[ROICandidate]:
...
def _fill_gap_between(
    merged_mask: np.ndarray, group_a: ROICandidate, group_b: ROICandidate, img_shape: tuple
) -> np.ndarray:
...
def _try_bridge_roi(group_indices, groups, comp_mask, solder_mask, img_shape, fill_gap, bridge_max_cut_ratio):
...
def get_bridge_candidates(
    ...
    groups = get_bridge_groups(components, img_shape, classes=bridge_classes, min_area_abs=min_area_abs)
    if len(groups) < 2:
        return []

    comp_mask = merge_class_masks(components, ("capacitor", "ic"), img_shape)
    solder_mask = merge_class_masks(components, ("solder",), img_shape)
    n = len(groups)
    candidates = []
    seen = set()
    ...
    for i in range(n):
        for j in range(i + 1, n):
            key = (i, j)
            cand = _try_bridge_roi(key, groups, comp_mask, solder_mask, img_shape, fill_gap, bridge_max_cut_ratio)
            if cand:
    ...
    if max_chain >= 3:
        for i, j in valid_pairs:
            for k in range(n):
                ...
                if triple in seen:
                    continue
                cand = _try_bridge_roi(
                    triple, groups, comp_mask, solder_mask, img_shape, fill_gap, bridge_max_cut_ratio
                )
                if cand:
                    seen.add(triple)
                    candidates.append(cand)
...
class BridgeMaskPlacer:
    ...
    def _make_expanded_roi(self, roi_mask):
        expanded = dilate(roi_mask, self.expand_kernel_size, iterations=self.expand_iterations)
    ...
    def place_all(self, submask_path, bridge_candidates, groups, img_shape, n_instances=-1, n_seeds=5, base_seed=42):
        ...
        for seed_i in range(n_seeds):
            ...
            log.info(f"Bridge seed {seed_i + 1}: requested={n_sel}, ok={n_ok}, fallback={n_fallback}")
        return results
