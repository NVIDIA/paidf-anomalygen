...
def _blend_overlay(img, mask, color, alpha=0.6):
    ...
    out = img.copy()
    roi = mask > 0
    out[roi] = (out[roi].astype(np.float32) * (1 - alpha) + np.array(color, dtype=np.float32) * alpha).astype(np.uint8)
    return out
...
def _make_defect_panel(cad_img, cands, color, annotate=True):
    ...
    for i, cand in enumerate(cands):
        ...
        if annotate:
            ...
            cv2.putText(overlay, str(i), (cx + 4, cy - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255, 255, 255), 1)
    return overlay


def visualize_roi_candidates(cad_mask_path: str, candidates: dict, output_dir: str, mask_name: str = "0000"):
    ...
    for defect_type in ("missing", "less_solder", "bridge"):
        ...
        cv2.putText(panel, defect_type, (2, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255, 255, 255), 1)
        panels.append(panel)
