...
from anomalygen.auto_mask_placement.data_types import ROI, BoundingBox
...
class ROISeparator:
    ...
    def add_roi(self, bbox: BoundingBox, is_legal: bool, roi_id: str = None):
        ...
        if (
            bbox.x < 0
            or bbox.y < 0
            or bbox.x + bbox.width > self.image_width
            or bbox.y + bbox.height > self.image_height
        ):
    ...
    def create_separated_roi_masks_from_json(self) -> List[np.ndarray]:
        ...
        log.info(
            f"Loaded {len(original_legal_masks)} legal ROIs and {np.sum(illegal_mask > 127)} illegal pixels from JSON definitions"
        )
