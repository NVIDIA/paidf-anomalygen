...
class MultiRankCheckpointer(BaseCheckpointer):
    ...
    def load(
        ...
        if checkpoint_path is not None:
            ...
            state_dict = torch.load(checkpoint_path, map_location=lambda storage, loc: storage, weights_only=True)
            ...
            if resume:
                ...
                optimizer.load_state_dict(state_dict["optimizer"], weights_only=True)
                log.info("- Loading the scheduler...")
                scheduler.load_state_dict(state_dict["scheduler"], weights_only=True)
                scheduler.last_epoch = iteration
                log.info("- Loading the gradient scaler...")
                grad_scaler.load_state_dict(state_dict["grad_scaler"], weights_only=True)
