...
from cosmos_predict2.configs.anomaly_gen.callbacks import register_callbacks
from cosmos_predict2.configs.anomaly_gen.checkpoint import register_checkpoint
from cosmos_predict2.configs.anomaly_gen.early_stop import EarlyStopTrainerConfig, register_early_stop
from cosmos_predict2.configs.anomaly_gen.model import register_model
...
from imaginaire.ag_trainer import AnomalyGenTrainer as Trainer
...
class Config(config.Config):
    ...
    defaults: List[Any] = attrs.field(
        factory=lambda: [
            ...
            {"scheduler": "constant"},
            {"model": "predict2_anomaly_gen_ddp_2b"},
            {"callbacks": ["basic"]},
            {"net": None},
            {"ema": None},
            {"checkpoint": None},
    ...
    # Override parent's trainer field type so config.trainer has an `early_stop` slot.
    trainer: EarlyStopTrainerConfig = attrs.field(factory=EarlyStopTrainerConfig)
