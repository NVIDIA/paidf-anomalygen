...
class CosmosT5TextEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        Args:
            model_name: The name of the T5 model to use.  Also used as the
                fallback HuggingFace hub ID when *cache_dir* fails to load.
            device: The device to use for computations.
            cache_dir: Local directory containing a pretrained T5 checkpoint.
            local_files_only: If True, only load from local files.
        """
        super().__init__()
        try:
        ...
        self.device = device
        log.info(
            f"Loaded T5 encoder: d_model={self.text_encoder.config.d_model}, "
            f"num_layers={self.text_encoder.config.num_layers}, "
            f"d_ff={self.text_encoder.config.d_ff}"
        )
