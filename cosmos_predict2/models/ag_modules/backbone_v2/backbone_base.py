...
from cosmos_predict2.models.ag_modules.backbone_v2.nn.norm import FrozenBatchNorm2d
...
class BackboneBase(nn.Module, metaclass=BackboneMeta):
    ...
    def freeze_backbone(self):
        ...
        if self.freeze_norm:
            ...
            log.warning("All batch normalization layers are frozen.")

        if self.freeze_at == "all":
            self._freeze_module(self)
            log.warning("The backbone is frozen.")
            ...
            for key in self.freeze_at:
                if key in stage_dict:
                    ...
                    log.warning(f"Stage {key} is frozen.")
                else:
                    log.warning(f"Stage {key} not found. Freezing options: {stage_dict.keys()}")
