...
from imaginaire.utils.graph_training import create_cuda_graph_training
...
class MiniTrainDIT(WeightTrainingStat):
    ...
    def forward(
        ...
        if use_cuda_graphs:
            if torch.is_grad_enabled():
                for i, block in enumerate(self.blocks):
                    hooks = {
                        "forward": len(block._forward_hooks),
                        "backward": len(block._backward_hooks),
                        "forward_pre": len(block._forward_pre_hooks),
                    }
                    active = {k: v for k, v in hooks.items() if v > 0}
                    if active:
                        raise RuntimeError(
                            f"DiT block {i} has active hooks {active} incompatible with "
                            f"CUDA graphs. Disable SAC and/or FSDP on DiT blocks first."
                        )
                shapes_key = create_cuda_graph_training(
                    self.cuda_graphs_training,
                    self.blocks,
                    x_B_T_H_W_D,
                    t_embedding_B_T_D,
                ...
                blocks = self.cuda_graphs_training[shapes_key]
            else:
        ...
        if use_cuda_graphs and not getattr(self, "_cuda_graph_validated", True):
            self._cuda_graph_validated = True
            self.validate_cuda_graph_correctness(
    ...
    @torch.no_grad()
    def _validate_cuda_graph_forward(self, x, t, c, kw):
        """Forward-only comparison: graphed replay vs original Block.forward."""
        x_g = x.detach().clone()
        x_r = x.detach().clone()
        for i, block in enumerate(self.blocks):
            x_g = block(x_g, t, c, **kw)
            x_r = Block.forward(block, x_r, t, c, **kw)
            d = (x_g.float() - x_r.float()).abs()
            scale = x_r.float().abs().max().item() + 1e-8
            log.critical(
                f"  Block {i:3d}  fwd  max_abs={d.max().item():.4e}  "
                f"mean={d.mean().item():.4e}  max_rel={d.max().item()/scale:.4e}"
            )

    def _validate_cuda_graph_backward(self, x, t, c, kw):
        """Forward+backward comparison: graphed vs original, checking dx and param grads."""
        t_d = t.detach()
        c_d = c.detach()
        kw_d = {k: (v.detach() if isinstance(v, torch.Tensor) else v) for k, v in kw.items()}

        trainable = [(n, p) for n, p in self.named_parameters() if p.requires_grad]

        def _run_path(x_in, use_graph):
            """Run all blocks and backward, return (dx, param_grads)."""
            xi = x_in
            if use_graph:
                for block in self.blocks:
                    xi = block(xi, t_d, c_d, **kw_d)
            else:
                for block in self.blocks:
                    xi = Block.forward(block, xi, t_d, c_d, **kw_d)
            for _, p in trainable:
                if p.grad is not None:
                    p.grad.zero_()
            xi.sum().backward()
            pgrads = {n: p.grad.detach().clone() for n, p in trainable if p.grad is not None}
            dx = x_in.grad.detach().clone() if x_in.grad is not None else None
            return dx, pgrads

        x_g = x.detach().clone().requires_grad_(True)
        dx_g, pg_g = _run_path(x_g, use_graph=True)

        x_r = x.detach().clone().requires_grad_(True)
        dx_r, pg_r = _run_path(x_r, use_graph=False)

        if dx_g is not None and dx_r is not None:
            dd = (dx_g.float() - dx_r.float()).abs()
            s = dx_r.float().abs().max().item() + 1e-8
            log.critical(
                f"  dx       bwd  max_abs={dd.max().item():.4e}  "
                f"mean={dd.mean().item():.4e}  max_rel={dd.max().item()/s:.4e}"
            )
        else:
            log.critical(
                f"  dx  graph={'OK' if dx_g is not None else 'None'}  "
                f"ref={'OK' if dx_r is not None else 'None'}"
            )

        worst_name, worst_abs, worst_rel = "", 0.0, 0.0
        n_large = 0
        for name in sorted(pg_g):
            if name not in pg_r:
                continue
            dd = (pg_g[name].float() - pg_r[name].float()).abs()
            ma = dd.max().item()
            s = pg_r[name].float().abs().max().item() + 1e-8
            rel = ma / s
            if ma > worst_abs:
                worst_abs, worst_name, worst_rel = ma, name, rel
            if rel > 0.01:
                n_large += 1
                log.critical(f"  PARAM >1%  {name}  abs={ma:.4e}  rel={rel:.4e}")

        log.critical(
            f"  Param grad summary:  worst={worst_name}  "
            f"max_abs={worst_abs:.4e}  max_rel={worst_rel:.4e}  "
            f"params_with_>1%_diff={n_large}/{len(pg_g)}"
        )
        if n_large == 0:
            log.critical("  All parameter gradients match within 1% relative tolerance.")

        for _, p in trainable:
            if p.grad is not None:
                p.grad.zero_()

    def validate_cuda_graph_correctness(self, x, t_embedding, crossattn_emb, block_kwargs):
        """Public entry: run forward and backward correctness checks for CUDA-graphed blocks.

        Compares the TE-graphed replay path against calling the original Block.forward
        directly (bypassing the CUDA graph).  Logs per-block forward diffs, input-gradient
        diffs, and per-parameter gradient diffs.
        """
        log.critical("=" * 70)
        log.critical("  CUDA GRAPH CORRECTNESS VALIDATION  (blocks only, one-time)")
        log.critical("=" * 70)

        log.critical("--- Forward pass (per-block) ---")
        t_d = t_embedding.detach()
        c_d = crossattn_emb.detach()
        kw_d = {k: (v.detach() if isinstance(v, torch.Tensor) else v) for k, v in block_kwargs.items()}
        self._validate_cuda_graph_forward(x, t_d, c_d, kw_d)

        log.critical("--- Backward pass ---")
        self._validate_cuda_graph_backward(x, t_embedding, crossattn_emb, block_kwargs)
    ...
    def disable_selective_checkpoint(self) -> None:
        from torch.distributed.algorithms._checkpoint.checkpoint_wrapper import CheckpointWrapper

        for block_id, block in self.blocks.named_children():
            if isinstance(block, CheckpointWrapper):
                self.blocks.register_module(block_id, block._checkpoint_wrapped_module)
        if isinstance(self.final_layer, CheckpointWrapper):
            self.register_module("final_layer", self.final_layer._checkpoint_wrapped_module)
