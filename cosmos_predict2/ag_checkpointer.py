...
from cosmos_predict2.checkpointer import Checkpointer as CheckpointerBase
...
class Checkpointer(CheckpointerBase):
    """The checkpointer class for Cosmos Anomaly Gen. Supports checkpoint saving/loading to local disk."""
    ...
    def save(
        ...
        state_dicts_to_save = {
            "model": model.state_dict(), # since state_dict returns only ag's state dict, saving model.state_dict() is enough
    ...
    def load(
        ...
        if latest_checkpoint_file is not None:
            ...
            for key, checkpoint_path in state_dicts_paths.items():
                ...
                state_dicts_to_load[key] = torch.load(checkpoint_path, map_location=lambda storage, loc: storage, weights_only=True)
            ...
            # Load the state dicts. Using model's load_state_dict to load ag's state dict only
            log.info("- Loading the model...")
            model.load_state_dict(state_dicts_to_load["model"], strict=self.strict_resume) # We load ag's state dict only
            ...
            if resume:
                ...
                # TODO: `set_optimizer_state_dict` will cause `KeyError` in the
                # anomaly embeddings. It is fine to use `load_state_dict`
                # since the trainable variables are limited.
