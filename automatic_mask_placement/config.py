...
class AugmentationParams:
    ...
    def from_config_file(cls, config_path: str) -> 'AugmentationParams':
        ...
        if invalid_keys:
            raise ValueError(f"Invalid keys found in config file '{config_path}': {sorted(invalid_keys)}. "
                           f"Valid keys are: {sorted(valid_config_keys)}")
        
        # Convert lists to tuples for range fields (JSON deserializes tuples as lists)
        tuple_fields = [
            'shift_x_range', 'shift_y_range', 'rotation_range',
            'scale_x_range', 'scale_y_range', 'shear_x_range', 'shear_y_range'
        ]
