...
from automatic_mask_placement.cad2roi.mask_utils import load_and_scale_submask, save_temp_mask, read_amp_output, mask_area
from automatic_mask_placement.cad2roi.placement_checks import clip_to_roi, check_min_area
...
def _validate_placement(placed, roi_mask, roi_area, min_area_ratio):
    ...
    if mask_area(clipped) == 0:
        return None
    if not check_min_area(clipped, roi_area, min_area_ratio):
        return None
    return clipped
...
class LessSolderMaskPlacer:
    ...
    def __init__(self, scale_factor: float = 0.8, min_area_ratio: float = 0.5,
                 max_retries: int = 100):
        self.scale_factor = scale_factor
        self.min_area_ratio = min_area_ratio
        self.max_retries = max_retries

    def place_on_roi(self, submask_path: str, roi_candidate: ROICandidate,
                     img_shape: tuple, seed: int = 42) -> Tuple[Optional[np.ndarray], bool]:
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_dir:
                for attempt in range(self.max_retries):
                    ...
                    clipped = _validate_placement(
                        placed, cand.mask, cand.area, self.min_area_ratio)
                    if clipped is None:
                        continue
        ...
        for seed_i in range(n_seeds):
            ...
            for ri in roi_indices:
                mask, is_fallback = self.place_on_roi(
                    submask_path, candidates[ri], img_shape,
                    seed=seed + ri * 100,
                )
                if mask is not None:
            ...
            log.info(f"LessSolder seed {seed_i+1}: requested={n_sel}, "
