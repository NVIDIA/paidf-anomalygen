...
How to obtain a ready-to-use `dataset_dir` for each supported use case (UC).
...
UC1 and UC3 fetch from Hugging Face — make sure `HF_TOKEN` is exported (or
`hf auth login` has been run) with read access to the relevant
`nvidia/Cosmos-AnomalyGen-*` dataset repos.
...
The script selects a curated subset (five anomaly images + masks per type, 20 clean images)
...
## Finetuned AnomalyGen checkpoints (optional — for `mode=inference_only`)

The 2B Cosmos-Predict2 finetunes for each UC are published on Hugging Face and
can be downloaded with a helper script:
...
pipeline at the downloaded directory via `checkpoint_dir=`.
