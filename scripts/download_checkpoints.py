...
MD5_CHECKSUM_LOOKUP = {
    ...
    "google-t5/t5-large/pytorch_model.bin": "9dd908dea3d9281b84aace3b7388070e",
    # Cosmos-Guardrail1
    ...
    # Qwen3-VL
    "Qwen/Qwen3-VL-4B-Instruct/model-00001-of-00002.safetensors": "c591eca9f96dd4410d6772da2d2c1c1b",
    "Qwen/Qwen3-VL-4B-Instruct/model-00002-of-00002.safetensors": "9c9c88728c44a09c57fdd04ea6ac5464",
}
...
def main(args):
    ...
    if "text2image" in args.model_types:
        for size in args.model_sizes:
            ...
            print(f"Downloading {repo_id} to {args.checkpoint_dir}...")
    ...
    # Download NVDINOv2 model
    nvdinov2_local_dir = os.path.join(args.checkpoint_dir, "NVDINOV2")
    os.makedirs(nvdinov2_local_dir, exist_ok=True)
    try:
        subprocess.run([
            "wget",
            "--content-disposition", "https://api.ngc.nvidia.com/v2/models/org/nvidia/team/tao/nv_dinov2_classification_model/trainable_v1.1/files?redirect=true&path=nv_dinov2_classification_model.ckpt",
            "--output-document", f"{nvdinov2_local_dir}/nv_dinov2_classification_model.ckpt",
        ])
        print("\033[92mSuccessfully downloaded NVDINOv2\033[0m")
    except Exception as e:
        print(f"\033[91mError downloading NVDINOv2: {e}\033[0m")

    # Download C-RADIOv3-B model
    cradiov3_local_dir = os.path.join(args.checkpoint_dir, "nvidia/C-RADIO-V3")
    os.makedirs(cradiov3_local_dir, exist_ok=True)
    hf_hub_download(
        repo_id="nvidia/C-RADIOv3-B", filename="model.safetensors", local_dir=cradiov3_local_dir
    )

    # Download facebook/dinov2-large model
    dinov2_large_local_dir = os.path.join(args.checkpoint_dir, "facebook/dinov2-large")
    os.makedirs(dinov2_large_local_dir, exist_ok=True)
    try:
        snapshot_download(repo_id="facebook/dinov2-large", local_dir=dinov2_large_local_dir)
        print("\033[92mSuccessfully downloaded facebook/dinov2-large\033[0m")
    except Exception as e:
        print(f"\033[91mError downloading facebook/dinov2-large: {e}\033[0m")

    # Download SAM2 model
    sam2_local_dir = os.path.join(args.checkpoint_dir, "sam2")
    os.makedirs(sam2_local_dir, exist_ok=True)
    sam2_path = os.path.join(sam2_local_dir, "sam2.1_hiera_large.pt")
    if os.path.exists(sam2_path):
        print(f"\033[92mSAM2 checkpoint already exists at {sam2_path}\033[0m")
    else:
        try:
            subprocess.run([
                "wget",
                "--output-document", sam2_path,
                "https://dl.fbaipublicfiles.com/segment_anything_2/092824/sam2.1_hiera_large.pt",
            ], check=True)
            print("\033[92mSuccessfully downloaded SAM2\033[0m")
        except Exception as e:
            print(f"\033[91mError downloading SAM2: {e}\033[0m")

    # Download Qwen3-VL model
    download_model(args.checkpoint_dir, "Qwen/Qwen3-VL-4B-Instruct", verify_md5=args.verify_md5)
