...
from anomalygen.auto_mask_placement import (
    ROI,
    AlignmentPoint,
    AugmentationParams,
    AutoMaskPlacement,
...
def _create_test_mask(size=(100, 100), shape="rectangle", position="center"):
    """
...
def _create_roi_mask(size=(512, 512), roi_boxes=None):
    """
...
def _create_multi_roi_mask(size=(512, 512), n_rois=3, min_area=1000):
    """
...
def _compute_coverage(placed_mask: np.ndarray, roi_mask: np.ndarray) -> float:
    """
...
class TestAugmentationParams:
    ...
    def test_custom_ranges_disable_dynamic_mode(self):
        ...
        try:
            ...
            assert not loaded_params.use_dynamic_shift_range, (
        ...
        try:
            ...
            assert not loaded_params.use_dynamic_rotation_range, (
        ...
        try:
            ...
            assert not loaded_params.use_dynamic_shear_range, (
        ...
        try:
            loaded_params = AugmentationParams.from_config_file(temp_path)
            assert loaded_params.use_dynamic_shift_range, (
                "Dynamic shift should remain enabled when no range is specified"
            )
            assert loaded_params.use_dynamic_rotation_range, (
                "Dynamic rotation should remain enabled when no range is specified"
            )
            assert loaded_params.use_dynamic_shear_range, (
...
class TestMaskAugmentor:
    ...
    def test_augmentor_with_custom_alignment(self):
        ...
        augmentor = MaskAugmentor(
            params, roi_alignment_point=AlignmentPoint.TOP_LEFT, submask_alignment_point=AlignmentPoint.BOTTOM_CENTER
        )
    ...
    def test_augment_mask_returns_tuple(self):
        ...
        params = AugmentationParams()
        augmentor = MaskAugmentor(params)
        mask = _create_test_mask(size=(50, 50))
...
class TestMaskPlacer:
    ...
    def test_mask_placer_custom_alignment(self):
        ...
        placer = MaskPlacer(image_width=512, image_height=512, roi_alignment_point=AlignmentPoint.TOP_LEFT)
    ...
    def test_place_mask_success(self):
        ...
        roi_mask = _create_roi_mask(size=(512, 512), roi_boxes=[(200, 200, 100, 100)])
    ...
    def test_place_mask_clipped_by_roi(self):
        ...
        placer = MaskPlacer(image_width=512, image_height=512, roi_alignment_point=AlignmentPoint.CENTER)
        ...
        roi_mask = _create_roi_mask(size=(512, 512), roi_boxes=[(230, 230, 50, 50)])
    ...
    def test_calculate_alignment_position_center(self):
        ...
        roi_mask = _create_roi_mask(size=(512, 512), roi_boxes=[(100, 100, 200, 200)])
        fixed_point = (25, 25)
...
class TestROISeparator:
    ...
    def test_separate_connected_regions(self):
        ...
        roi_img = _create_multi_roi_mask(size=(512, 512), n_rois=3, min_area=1000)
...
# =============================================================================
# Test AutoMaskPlacement (Integration Tests)
# =============================================================================


class TestAutomaticMaskPlacement:
    """Integration tests for AutoMaskPlacement main class"""

    def test_amp_creation(self):
        """Test AutoMaskPlacement creation"""
    ...
    def test_amp_with_custom_params(self):
        ...
        params = AugmentationParams(shift_x_probability=0.5, rotation_range=(-10, 10))

        amp = AutoMaskPlacement(image_width=1024, image_height=768, augmentation_params=params, random_seed=123)
    ...
    def test_amp_load_combined_rois_from_image(self):
        ...
        amp = AutoMaskPlacement(
            image_width=512, image_height=512, augmentation_params=AugmentationParams(), random_seed=42
        )
    ...
    def test_amp_process_submask(self):
        ...
        roi_mask = _create_roi_mask(size=(512, 512), roi_boxes=[(100, 100, 300, 300)])
        submask = _create_test_mask(size=(50, 50), shape="rectangle")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            results = amp.process_submask(submask_path=submask_path, n_instances=3, output_dir=output_dir)
    ...
    def test_amp_deterministic_with_seed(self):
        ...
        # Create test data
        roi_mask = _create_roi_mask(size=(256, 256), roi_boxes=[(50, 50, 150, 150)])
        submask = _create_test_mask(size=(30, 30), shape="rectangle")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # First run
            amp1 = AutoMaskPlacement(image_width=256, image_height=256, augmentation_params=params, random_seed=42)
            amp1.load_combined_rois(roi_image_paths=[roi_path])
            results1 = amp1.process_submask(submask_path=submask_path, n_instances=2, output_dir=output_dir1)

            # Second run with same seed
            amp2 = AutoMaskPlacement(image_width=256, image_height=256, augmentation_params=params, random_seed=42)
            amp2.load_combined_rois(roi_image_paths=[roi_path])
            results2 = amp2.process_submask(submask_path=submask_path, n_instances=2, output_dir=output_dir2)
...
class TestEdgeCases:
    ...
    def test_mask_at_image_boundary(self):
        ...
        # ROI at corner
        roi_mask = _create_roi_mask(size=(100, 100), roi_boxes=[(0, 0, 30, 30)])
    ...
    def test_non_square_dimensions(self):
        ...
        amp = AutoMaskPlacement(
            image_width=1920, image_height=1080, augmentation_params=AugmentationParams(), random_seed=42
        )
...
class TestAlignmentModes:
    ...
    @pytest.mark.parametrize(
        "alignment_point,expected_region",
        [
    ...
    def test_mask_placer_alignment_modes(self, alignment_point, expected_region):
        ...
        placer = MaskPlacer(image_width=512, image_height=512, roi_alignment_point=alignment_point)
        ...
        roi_mask = _create_roi_mask(size=(512, 512), roi_boxes=[(156, 156, 200, 200)])
    ...
    @pytest.mark.parametrize(
        "submask_alignment,expected_fixed_point",
        [
...
class TestAugmentationOperations:
    ...
    @pytest.mark.parametrize(
        "operation,param_name,param_value",
        [
    ...
    def test_individual_augmentation_operations(self, operation, param_name, param_value):
        ...
        mask = _create_test_mask(size=(50, 50), shape="rectangle")
        augmented, fixed_point = augmentor.augment_mask(mask)
...
class TestImageSizes:
    ...
    def test_different_image_sizes(self, image_size):
        ...
        amp = AutoMaskPlacement(image_width=w, image_height=h, augmentation_params=AugmentationParams(), random_seed=42)
    ...
    def test_different_mask_and_roi_sizes(self, submask_size, roi_size):
        ...
        # Create ROI
        roi_mask = _create_roi_mask(size=(512, 512), roi_boxes=[(256 - roi_w // 2, 256 - roi_h // 2, roi_w, roi_h)])
...
class TestIntegrationWithIoU:
    ...
    def test_amp_generates_combined_mask(self, n_instances):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            # Create test data
            roi_mask = _create_roi_mask(size=(512, 512), roi_boxes=[(100, 100, 300, 300)])
            submask = _create_test_mask(size=(40, 40), shape="rectangle")
            ...
            amp = AutoMaskPlacement(image_width=512, image_height=512, augmentation_params=params, random_seed=42)
            amp.load_combined_rois(roi_image_paths=[roi_path])

            results = amp.process_submask(submask_path=submask_path, n_instances=n_instances, output_dir=output_dir)
    ...
    def test_placed_masks_within_roi(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            roi_mask = _create_roi_mask(size=(512, 512), roi_boxes=[(50, 50, 400, 400)])
            submask = _create_test_mask(size=(30, 30), shape="rectangle")
            ...
            results = amp.process_submask(submask_path=submask_path, n_instances=5, output_dir=output_dir)

            # Verify each placed mask is within ROI
            for result in results:
                placed_mask = np.array(Image.open(result["output_path"]))
                coverage = _compute_coverage(placed_mask, roi_mask)
    ...
    def test_masks_have_sufficient_pixels(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for result in results:
                ...
                assert pixel_count >= min_pixel_count, f"Mask has too few pixels: {pixel_count} < {min_pixel_count}"
...
def test_amp_pipeline(description, image_size, roi_config, submask_config, aug_config, n_instances):
    """Comprehensive parametrized pipeline test for AutoMaskPlacement.
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        # Create ROI mask
        roi_mask = _create_roi_mask(size=(height, width), roi_boxes=roi_config["boxes"])

        # Create submask
        submask = _create_test_mask(size=submask_config["size"], shape=submask_config["shape"])
        ...
        # Run AMP
        amp = AutoMaskPlacement(image_width=width, image_height=height, augmentation_params=params, random_seed=42)
        amp.load_combined_rois(roi_image_paths=[roi_path])
        ...
        assert all(v in [0, 255] for v in unique_values), f"{description}: Mask is not binary: {unique_values}"
